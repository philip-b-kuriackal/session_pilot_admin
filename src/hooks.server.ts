...
const authGuard: Handle = async ({ event, resolve }) => {
	...
	const isPublic = path === '/login' || path.startsWith('/auth') || path.startsWith('/display') || path.startsWith('/api/mobile-checkin') || path.startsWith('/api/user-exists') || path.startsWith('/api/urgent-messages');
