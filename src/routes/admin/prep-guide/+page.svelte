...
<div class="page-head">
	<div>
		<h1>Prep Guide</h1>
		<p>Write the Prep Guide for each location to appear in the Academy in the mobile app.</p>
	</div>
</div>

<div class="card">
	<h2>Edit Prep Guide</h2>
	...
	>
		...
		{#if selectedLocation}
			...
			<div class="form-grid" style="grid-template-columns: 1fr;">
				<label class="field">
					<span>Title *</span>
					<input type="text" name="title" required placeholder="e.g. Prep Guide" value={existingEntry?.title || ''} />
				</label>
			</div>

			<div class="form-grid" style="grid-template-columns: 1fr;">
				<label class="field">
					<span>Details / Guides (one per line)</span>
					<textarea name="content_bullets" rows="8" placeholder="Task 1...&#10;Task 2...">{existingEntry?.content_bullets || ''}</textarea>
				</label>
			</div>
	...
	{#if selectedLocation}
		<div class="form-actions" style="margin-top: 1.5rem; display: flex; justify-content: space-between;">
			<div>
				{#if existingEntry}
					<form method="POST" action="?/delete" use:enhance onsubmit={confirmSubmit("Delete this Prep Guide?")}>
						<input type="hidden" name="id" value={existingEntry.id} />
