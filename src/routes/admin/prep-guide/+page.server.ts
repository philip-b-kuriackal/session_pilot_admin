...
export const actions: Actions = {
	save: async ({ request, locals }) => {
		...
		const svc = serviceClient() || locals.supabase;
		const { error } = await svc.from('prep_guides').upsert(dataToUpsert);
		...
		return { success: true, message: 'Prep Guide saved!' };
	},
	delete: async ({ request, locals }) => {
		...
		if (id) {
			const svc = serviceClient() || locals.supabase;
			await svc.from('prep_guides').delete().eq('id', id);
		}
		return { success: true, message: 'Prep Guide deleted' };
	}
};
