...
export const actions: Actions = {
	save: async ({ request, locals }) => {
		...
		const svc = serviceClient() || locals.supabase;
		const { error } = await svc.from('safety_guidelines').upsert(dataToUpsert);
		...
		return { success: true, message: 'Safety Guidelines saved!' };
	},
	delete: async ({ request, locals }) => {
		...
		if (id) {
			const svc = serviceClient() || locals.supabase;
			await svc.from('safety_guidelines').delete().eq('id', id);
		}
		return { success: true, message: 'Safety Guidelines deleted' };
	}
};
