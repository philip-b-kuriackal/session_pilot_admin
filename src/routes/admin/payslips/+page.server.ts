...
export const actions: Actions = {
	save: async ({ request, locals }) => {
		...
		const svc = serviceClient() || locals.supabase;
		const { error } = await svc.from('payslips_info').upsert(dataToUpsert);
		...
		return { success: true, message: 'Payslips Info saved!' };
	},
	delete: async ({ request, locals }) => {
		...
		if (id) {
			const svc = serviceClient() || locals.supabase;
			await svc.from('payslips_info').delete().eq('id', id);
		}
		return { success: true, message: 'Payslips Info deleted' };
	}
};
