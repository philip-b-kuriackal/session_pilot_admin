...
<div class="page-head">
	<div>
		<h1>Payslips Info</h1>
		<p>Write the Payslips Info for each location to appear in the Tools tab in the mobile app.</p>
	</div>
</div>

<div class="card">
	<h2>Edit Payslips Info</h2>
	...
	>
		...
		{#if selectedLocation}
			...
			<div class="form-grid" style="grid-template-columns: 1fr;">
				<label class="field">
					<span>Title *</span>
					<input type="text" name="title" required placeholder="e.g. Understanding Your Payslip" value={existingEntry?.title || ''} />
				</label>
			</div>

			<div class="form-grid" style="grid-template-columns: 1fr;">
				<label class="field">
					<span>Details / Info (one per line)</span>
					<textarea name="content_bullets" rows="8" placeholder="Info 1...&#10;Info 2...">{existingEntry?.content_bullets || ''}</textarea>
				</label>
			</div>
			{/key}
		{/if}
	...
	{#if selectedLocation}
		<div class="form-actions" style="margin-top: 1.5rem; display: flex; justify-content: space-between;">
			<div>
				{#if existingEntry}
					<form method="POST" action="?/delete" use:enhance onsubmit={confirmSubmit("Delete this Payslips Info?")}>
						<input type="hidden" name="id" value={existingEntry.id} />
