...
<div class="page-head">
	<div>
		...
		<p>Write the Code of Conduct for each location to appear in the Hub Handbook in the mobile app.</p>
	</div>
</div>
...
<div class="card">
	...
	>
		...
		{#if selectedLocation}
			...
			<div class="form-grid" style="grid-template-columns: 1fr;">
				<label class="field">
					<span>Title *</span>
					<input type="text" name="title" required placeholder="e.g. Code of Conduct" value={existingEntry?.title || ''} />
				</label>
			</div>

			<div class="form-grid" style="grid-template-columns: 1fr;">
				<label class="field">
					<span>Details / Policies (one per line)</span>
					<textarea name="content_bullets" rows="8" placeholder="Policy 1...&#10;Policy 2...">{existingEntry?.content_bullets || ''}</textarea>
				</label>
			</div>
	...
	{#if selectedLocation}
		<div class="form-actions" style="margin-top: 1.5rem; display: flex; justify-content: space-between;">
			<div>
				{#if existingEntry}
					<form method="POST" action="?/delete" use:enhance onsubmit={confirmSubmit("Delete this Code of Conduct?")}>
						<input type="hidden" name="id" value={existingEntry.id} />
