...
export const actions: Actions = {
	save: async ({ request, locals }) => {
		...
		const svc = serviceClient() || locals.supabase;
		const { error } = await svc.from('code_of_conducts').upsert(dataToUpsert);
		...
		return { success: true, message: 'Code of Conduct saved!' };
	},
	delete: async ({ request, locals }) => {
		...
		if (id) {
			const svc = serviceClient() || locals.supabase;
			await svc.from('code_of_conducts').delete().eq('id', id);
		}
		return { success: true, message: 'Code of Conduct deleted' };
	}
};
