...
export const actions: Actions = {
	save: async ({ request, locals }) => {
		...
		const svc = serviceClient() || locals.supabase;
		const { error } = await svc.from('benefits_info').upsert(dataToUpsert);
		...
		return { success: true, message: 'Benefits Info saved!' };
	},
	delete: async ({ request, locals }) => {
		...
		if (id) {
			const svc = serviceClient() || locals.supabase;
			await svc.from('benefits_info').delete().eq('id', id);
		}
		return { success: true, message: 'Benefits Info deleted' };
	}
};
