...
<div class="page-head">
	<div>
		<h1>Benefits Info</h1>
		<p>Write the Benefits Info for each location to appear in the Tools tab in the mobile app.</p>
	</div>
</div>

<div class="card">
	<h2>Edit Benefits Info</h2>
	...
	>
		...
		{#if selectedLocation}
			...
			<div class="form-grid" style="grid-template-columns: 1fr;">
				<label class="field">
					<span>Title *</span>
					<input type="text" name="title" required placeholder="e.g. Employee Benefits" value={existingEntry?.title || ''} />
				</label>
			</div>
			...
			<div class="form-grid" style="grid-template-columns: 1fr;">
				<label class="field">
					...
					<textarea name="content_bullets" rows="8" placeholder="Benefit 1...&#10;Benefit 2...">{existingEntry?.content_bullets || ''}</textarea>
				</label>
			</div>
			{/key}
		{/if}
	...
	{#if selectedLocation}
		<div class="form-actions" style="margin-top: 1.5rem; display: flex; justify-content: space-between;">
			<div>
				{#if existingEntry}
					<form method="POST" action="?/delete" use:enhance onsubmit={confirmSubmit("Delete this Benefits Info?")}>
						<input type="hidden" name="id" value={existingEntry.id} />
