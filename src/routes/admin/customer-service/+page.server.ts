...
export const actions: Actions = {
	save: async ({ request, locals }) => {
		...
		const svc = serviceClient() || locals.supabase;
		const { error } = await svc.from('customer_service_guides').upsert(dataToUpsert);
		...
		return { success: true, message: 'Customer Service saved!' };
	},
	delete: async ({ request, locals }) => {
		...
		if (id) {
			const svc = serviceClient() || locals.supabase;
			await svc.from('customer_service_guides').delete().eq('id', id);
		}
		return { success: true, message: 'Customer Service deleted' };
	}
};
