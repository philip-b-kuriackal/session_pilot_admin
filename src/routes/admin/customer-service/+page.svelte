...
<div class="page-head">
	<div>
		...
		<p>Write the Customer Service guidelines for each location to appear in the Academy in the mobile app.</p>
	</div>
</div>
...
<div class="card">
	...
	>
		...
		{#if selectedLocation}
			...
			<div class="form-grid" style="grid-template-columns: 1fr;">
				<label class="field">
					<span>Title *</span>
					<input type="text" name="title" required placeholder="e.g. Customer Service" value={existingEntry?.title || ''} />
				</label>
			</div>
	...
	{#if selectedLocation}
		<div class="form-actions" style="margin-top: 1.5rem; display: flex; justify-content: space-between;">
			<div>
				{#if existingEntry}
					<form method="POST" action="?/delete" use:enhance onsubmit={confirmSubmit("Delete these Customer Service guidelines?")}>
						<input type="hidden" name="id" value={existingEntry.id} />
