...
export const actions: Actions = {
	...
	createEvent: async ({ request, locals }) => {
		...
		if (profiles && profiles.length > 0) {
			...
			// Send Firebase Push Notifications via Topics
			if (profiles.length > 0) {
				try {
					const pushPromises = profiles.map(p => 
						sendPushNotification(
							'user_' + p.id,
							'New Event Scheduled!',
							`A new event "${title}" has been created. Tap to view upcoming events!`,
							'/events'
						)
					);
					await Promise.allSettled(pushPromises);
					console.log(`Sent Firebase push notifications to ${profiles.length} topics.`);
				} catch (err) {
