...
export const load: PageServerLoad = async ({ locals }) => {
	...
	if (!orgId) return { locations: [], needs: [] };
...
export const actions: Actions = {
	save: async ({ request, locals }) => {
		...
		const svc = serviceClient() || locals.supabase;
		const { error } = await svc.from('handbook_needs').upsert(dataToUpsert);
		...
		return { success: true, message: 'Needs list saved!' };
	},
	delete: async ({ request, locals }) => {
		...
		if (id) {
			const svc = serviceClient() || locals.supabase;
			await svc.from('handbook_needs').delete().eq('id', id);
		}
		return { success: true, message: 'Needs list deleted' };
	}
};
